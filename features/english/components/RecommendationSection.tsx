...
import type { EnglishRecommendation, RecommendationState, RecommendationType } from "../types";
...
export function RecommendationSection({ items, recommendationState, onToggleFavorite, onToggleWatched }: { items: EnglishRecommendation[]; recommendationState: Record<string, RecommendationState>; onToggleFavorite: (id: string) => void; onToggleWatched: (id: string) => void }) {
  ...
  const visibleItems: RecommendationItem[] = isOnline
    ...
    : activeTab === "speech"
      ? getDailySpeechRecommendations(sourceItems.filter((item): item is Extract<EnglishRecommendation, { type: "speech" }> => item.type === "speech"), dateKey)
      : activeTab === "movie"
        ? getWeeklyMovieRecommendations(sourceItems.filter((item): item is Extract<EnglishRecommendation, { type: "movie" }> => item.type === "movie"), dateKey)
        : getWeeklySeriesRecommendations(sourceItems.filter((item): item is Extract<EnglishRecommendation, { type: "series" }> => item.type === "series"), dateKey);
  return (
