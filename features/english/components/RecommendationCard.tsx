...
import type { RecommendationItem } from "../onlineTypes";
...
export function RecommendationCard({ item, state, onToggleFavorite, onToggleWatched }: { item: RecommendationItem; state?: RecommendationState; onToggleFavorite: (id: string) => void; onToggleWatched: (id: string) => void }) {
  const details = item.type === "speech"
    ? "detailText" in item ? item.detailText : `${item.speaker} · ${item.durationMinutes} 分钟`
    : item.type === "movie"
      ? "detailText" in item ? item.detailText : `${item.year} · ${item.genre} · ${item.durationMinutes} 分钟`
      : "detailText" in item ? item.detailText : `${item.genre} · 单集 ${item.episodeMinutes} 分钟 · ${item.seasons} 季`;
  const difficultyLabel = item.difficulty ? difficultyLabels[item.difficulty] : "在线推荐";
  const accentLabel = item.accent ? accentLabels[item.accent] : undefined;
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-card">
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="flex flex-wrap gap-2 text-[11px] font-semibold"><span className="rounded-lg bg-[#F1F0FF] px-2 py-1 text-[#5E5CE6]">{difficultyLabel}</span>{accentLabel ? <span className="rounded-lg bg-canvas px-2 py-1 text-muted">{accentLabel}</span> : null}{item.topics.slice(0, 1).map((topic) => <span key={topic} className="rounded-lg bg-canvas px-2 py-1 text-muted">{topic}</span>)}</div>
        ...
        {item.type === "speech" && <p className="text-[11px] text-muted">字幕：{item.captions ? `${item.captions.english ? "英文" : "无英文"}${item.captions.bilingual ? " · 中英双语" : ""}` : "以来源页面为准"}</p>}
