...
export function TodayPlan({ locale = "zh" }: { locale?: Locale }) {
  ...
  return <div className="mx-auto max-w-[1180px]">
    ...
    <section className="mb-9 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4"><StatCard label={isZh ? "今日任务" : "Today's tasks"} value={todayTasks.length} /><StatCard label={isZh ? "今日完成率" : "Completion rate"} value={`${completionRate}%`} /><StatCard label={isZh ? "已完成" : "Completed"} value={completedCount} /><StatCard label={isZh ? "待完成" : "Pending"} value={pendingCount} /></section>
    <section className={`grid items-stretch gap-6 ${isFormOpen ? "md:grid-cols-[minmax(0,1fr)_380px]" : "grid-cols-1"}`}>
      <div className="rounded-[24px] border border-line bg-white p-6 shadow-card sm:p-8">
        <div className="mb-8"><h2 className="text-lg font-bold">{view === "today" ? (isZh ? "今日待办" : "Today's tasks") : (isZh ? "历史任务" : "Task history")}</h2><p className="mt-1 text-xs text-muted">{view === "today" ? (isZh ? "专注于今天最重要的几件事" : "Focus on what matters today") : (isZh ? "回顾过去完成和未完成的计划" : "Review past plans")}</p><div className="mt-6 flex flex-wrap items-center justify-between gap-4"><div className="flex rounded-xl bg-canvas p-1"><TabButton active={view === "today"} onClick={() => setView("today")} label={isZh ? "今日任务" : "Today"} /><TabButton active={view === "history"} onClick={() => setView("history")} label={isZh ? "历史任务" : "History"} /></div><button onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-ink px-4 py-2.5 text-sm font-semibold text-white shadow-card transition hover:-translate-y-0.5"><Plus size={16} />{isZh ? "新增任务" : "Add task"}</button></div></div>
        {view === "today" && <div className="mb-8 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-[#E9EDF3]"><div className="h-full rounded-full bg-accent transition-all" style={{ width: `${completionRate}%` }} /></div><span className="min-w-[42px] text-right text-xs font-semibold text-muted">{completedCount} / {todayTasks.length}</span></div>}
        {visibleTasks.length > 0 ? <div className="space-y-4">{visibleTasks.map((task) => <TaskRow key={task.id} task={task} locale={locale} onToggle={toggleTask} onEdit={openEdit} onDelete={deleteTask} />)}</div> : <EmptyState view={view} locale={locale} onCreate={openCreate} />}
      </div>
...
function StatCard({ label, value }: { label: string; value: string | number }) { return <article className="grid min-h-[148px] place-items-center rounded-[20px] border border-line bg-white px-6 py-8 text-center shadow-card"><div className="w-full text-center"><p className="text-base font-semibold text-ink">{label}</p><p className="mt-3 text-4xl font-extrabold tracking-[-0.04em] text-ink">{value}</p></div></article>; }
...
function TaskForm({ task, locale, today, onClose, onSave }: { task: PlanTask | null; locale: Locale; today: string; onClose: () => void; onSave: (draft: TaskDraft) => void }) {
  ...
  return <aside className="flex h-full min-h-[560px] min-w-0 flex-col overflow-hidden rounded-[24px] border border-line bg-white shadow-card"><div className="flex shrink-0 items-start justify-between border-b border-line px-6 py-6 sm:px-8"><div><h2 className="text-xl font-bold">{task ? (isZh ? "编辑任务" : "Edit task") : (isZh ? "新增任务" : "Add task")}</h2><p className="mt-1 text-sm text-muted">{isZh ? "把任务信息补充完整，之后更容易执行。" : "Add context to make the task actionable."}</p></div><button onClick={onClose} className="grid size-9 place-items-center rounded-xl text-muted hover:bg-canvas" aria-label={isZh ? "关闭" : "Close"}><X size={18} /></button></div><form onSubmit={submit} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-6 sm:px-8"><Field label={isZh ? "任务标题" : "Title"} required><input autoFocus value={draft.title} onChange={(event) => update("title", event.target.value)} placeholder={isZh ? "例如：完成英语听力练习" : "e.g. Complete listening practice"} className="form-input" /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label={isZh ? "日期" : "Date"}><input type="date" value={draft.date} onChange={(event) => update("date", event.target.value)} className="form-input" /></Field><Field label={isZh ? "时间" : "Time"}><input type="time" value={draft.time} onChange={(event) => update("time", event.target.value)} className="form-input" /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label={isZh ? "优先级" : "Priority"}><select value={draft.priority} onChange={(event) => update("priority", event.target.value as TaskPriority)} className="form-input"><option value="low">{isZh ? "低" : "Low"}</option><option value="medium">{isZh ? "中" : "Medium"}</option><option value="high">{isZh ? "高" : "High"}</option></select></Field><Field label={isZh ? "分类" : "Category"}><select value={draft.category} onChange={(event) => update("category", event.target.value)} className="form-input">{categoryOptions.map((category) => <option key={category} value={category}>{category}</option>)}</select></Field></div><Field label={isZh ? "备注" : "Notes"}><textarea rows={4} value={draft.notes} onChange={(event) => update("notes", event.target.value)} placeholder={isZh ? "补充一些上下文或执行提醒" : "Add context or a reminder"} className="form-input resize-none" /></Field></form><div className="flex shrink-0 justify-end gap-3 border-t border-line bg-white px-6 py-5 sm:px-8"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-muted hover:bg-canvas">{isZh ? "取消" : "Cancel"}</button><button type="button" onClick={() => { if (draft.title.trim()) onSave({ ...draft, title: draft.title.trim() }); }} className="rounded-xl bg-ink px-5 py-2.5 text-sm font-semibold text-white">{task ? (isZh ? "保存修改" : "Save changes") : (isZh ? "创建任务" : "Create task")}</button></div></aside>;
}
