...
export function RestaurantSearch({ input, results, onChange, onSearch, onSelect, isSearching = false, selectedRestaurant, onOfficialEnrich, isOfficialEnriching = false }: RestaurantSearchProps) {
  ...
  return <section className="rounded-[24px] border border-[#E5D9CA] bg-[#FFFBF7] p-6 shadow-card sm:p-7">
    ...
    <form onSubmit={(event) => { event.preventDefault(); void onSearch(); }} className="mt-6 grid gap-3 md:grid-cols-[1.2fr_1fr_1fr_auto]"><input value={input.name} onChange={(event) => onChange({ ...input, name: event.target.value })} className="form-input" placeholder="餐厅名称" aria-label="餐厅名称" /><input value={input.address} onChange={(event) => onChange({ ...input, address: event.target.value })} className="form-input" placeholder="地址（可选）" aria-label="地址" /><input value={input.city} onChange={(event) => onChange({ ...input, city: event.target.value })} className="form-input" placeholder="城市（可选）" aria-label="城市" /><button type="submit" disabled={isSearching} className="flex items-center justify-center gap-2 rounded-xl bg-ink px-5 py-3 text-sm font-bold text-white transition hover:bg-[#2A2D32] disabled:cursor-not-allowed disabled:opacity-40"><Search size={16} />{isSearching ? "搜索中…" : "搜索"}</button></form>
