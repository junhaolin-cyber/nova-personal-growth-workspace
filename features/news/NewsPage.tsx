...
export function NewsPage() {
  ...
  return <div className="mx-auto max-w-[1400px] space-y-8">
    ...
    {mode === "favorites" || mode === "history" || mode === "tracking" ? <NewsCollections mode={mode} articles={favoriteArticles} history={state.history} trackedEvents={state.trackedEvents} onOpen={setSelectedArticleId} onRemoveFavorite={(articleId) => { const article = favoriteArticles.find((item) => item.id === articleId); if (article) toggleFavorite(article); }} onRemoveHistory={(articleId) => setState({ ...state, history: state.history.filter((item) => item.articleId !== articleId) })} onUntrack={(eventId) => setState({ ...state, trackedEvents: state.trackedEvents.filter((item) => item.eventId !== eventId) })} /> : null}
