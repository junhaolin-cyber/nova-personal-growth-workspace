    ...
    <div className="mx-auto max-w-[1440px]">
      ...
      <section className="mt-6">
        {view === "brands" ? <div className="grid gap-3 md:grid-cols-2">{visibleBrands.map((brand) => <TrendBrandCard key={brand.id} brand={brand} isFavorite={state.favoriteBrandIds.includes(brand.id)} onFavorite={() => toggleBrandFavorite(brand.id)} onOpen={() => recordBrandHistory(brand.id)} />)}</div> : <>
          ...
          {visibleItems.length > 0 ? <div className="grid grid-cols-2 gap-3 md:grid-cols-2 md:gap-4 xl:grid-cols-3">{visibleItems.map((item) => <TrendLifeCard key={item.id} item={item} coverUrl={coverByItemId[item.id]} isFavorite={state.favoriteIds.includes(item.id)} onFavorite={() => toggleFavorite(item.id)} onOpen={() => recordHistory(item.id)} />)}</div> : null}
        </>}
      </section>
