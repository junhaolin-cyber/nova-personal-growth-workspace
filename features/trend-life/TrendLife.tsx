...
function itemsForView(view: TrendLifeView, state: TrendLifeState, search: string): TrendItem[] {
...
export function TrendLife() {
  ...
  const visibleItems = React.useMemo(() => itemsForView(view, state, search), [view, state, search]);
  const visibleBrands = React.useMemo(() => brandsForView(view, state, search), [view, state, search]);
  const favoriteCount = state.favoriteIds.length + state.favoriteBrandIds.length;
  ...
  const toggleBrandFavorite = (id: string) => {
    setState((current) => ({
      ...current,
      favoriteBrandIds: current.favoriteBrandIds.includes(id)
        ? current.favoriteBrandIds.filter((item) => item !== id)
        : [...current.favoriteBrandIds, id],
    }));
  };
  ...
  const recordBrandHistory = (id: string) => {
    setState((current) => ({
      ...current,
  ...
  const hasCollectionContent = visibleItems.length > 0 || visibleBrands.length > 0;
  ...
  return (
    <div className="mx-auto max-w-[1440px]">
      <section className="rounded-[30px] border border-line bg-white p-6 shadow-card sm:p-8 lg:p-10">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-muted">
            <span className="rounded-full bg-[#F5F0FA] px-3 py-2 font-semibold text-[#805C9C]">{trendItems.length} 条精选内容</span>
            <span className="rounded-full bg-canvas px-3 py-2">{favoriteCount} 条收藏</span>
          </div>
        </div>
        <div className="mt-8 grid gap-4 lg:grid-cols-[1.4fr_1fr_1fr]">
          <div className="rounded-[24px] bg-[#1D1D22] p-6 text-white sm:p-7">
            <div className="flex items-start justify-between">
              <div><p className="text-xs text-white/60">今日潮流</p><h2 className="mt-3 text-2xl font-extrabold leading-9">让灵感先于<br />消费发生。</h2></div>
              <Compass size={25} className="text-[#D8C8F4]" />
            </div>
            <p className="mt-8 max-w-sm text-sm leading-6 text-white/70">优先浏览官方资讯和公开视频，不把潮流变成购物清单。</p>
          </div>
          ...
          <div className="rounded-[24px] bg-[#EEF5F2] p-6"><p className="text-xs font-bold text-[#4F8067]">今日穿搭</p><p className="mt-5 text-3xl font-extrabold text-[#263D31]">{trendItems.filter((item) => item.kind === "outfit").length}</p><p className="mt-2 text-sm text-[#668170]">条灵感整理</p></div>
        </div>
      </section>
      ...
      <section className="mt-6">
        {view === "brands" ? (
          <div className="grid gap-3 md:grid-cols-2">{visibleBrands.map((brand) => <TrendBrandCard key={brand.id} brand={brand} isFavorite={state.favoriteBrandIds.includes(brand.id)} onFavorite={() => toggleBrandFavorite(brand.id)} onOpen={() => recordBrandHistory(brand.id)} />)}</div>
        ) : (
          <>
            <div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-extrabold">{viewTabs.find((tab) => tab.id === view)?.label}</h2><p className="mt-1 text-sm text-muted">{view === "favorites" ? "你收藏的文章、视频、新品和品牌" : view === "history" ? "最近打开过的公开内容和品牌官网" : "轻量浏览，按你的兴趣慢慢收藏"}</p></div><div className="flex items-center gap-2 text-xs text-muted">{view === "videos" ? <Video size={15} /> : view === "history" ? <History size={15} /> : <Star size={15} />}{isCollectionView ? visibleItems.length + visibleBrands.length : visibleItems.length} 条</div></div>
      ...
      <p className="mt-8 text-xs leading-5 text-muted">内容以公开来源入口为主。潮流灵感不等于购买建议，页面不提供购物、价格、库存、尺码或抢购服务。</p>
    </div>
  );
}
