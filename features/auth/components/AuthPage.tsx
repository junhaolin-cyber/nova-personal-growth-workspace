      ...
      <section className="grid w-full overflow-hidden rounded-[32px] border border-line bg-white shadow-card lg:grid-cols-[.9fr_1.1fr]">
        ...
        <div className="p-6 sm:p-10 lg:p-12">
          ...
          {!(verificationEmail && notice) ? <form onSubmit={(event) => void handleSubmit(event)} className="space-y-4">
            {mode === "register" ? <Field label="昵称（可选）" icon={<UserRound size={16} />}><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="form-input !pl-12" placeholder="例如：Jie" maxLength={40} /></Field> : null}
            {mode !== "reset" ? <Field label="邮箱" icon={<Mail size={16} />}><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="form-input !pl-12" placeholder="you@example.com" autoComplete="email" required /></Field> : null}
...
function Field({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <label className="block text-sm font-semibold text-muted"><span className="mb-2 block">{label}</span><span className="relative block"><span className="pointer-events-none absolute inset-y-0 left-0 flex w-12 items-center justify-center text-[#A1A6AE]">{icon}</span>{children}</span></label>;
}
...
function PasswordField({ label, value, onChange, visible, onToggle, autoComplete }: { label: string; value: string; onChange: (value: string) => void; visible: boolean; onToggle: () => void; autoComplete: string }) {
  return <Field label={label} icon={<LockKeyhole size={16} />}><input type={visible ? "text" : "password"} value={value} onChange={(event) => onChange(event.target.value)} className="form-input !pl-12 !pr-14" placeholder="至少 6 位字符" autoComplete={autoComplete} required /><button type="button" onClick={onToggle} className="absolute right-1 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-lg text-muted hover:bg-canvas hover:text-accent" aria-label={visible ? "隐藏密码" : "显示密码"}>{visible ? <EyeOff size={16} /> : <Eye size={16} />}</button></Field>;
}
