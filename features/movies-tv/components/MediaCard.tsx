...
export function MediaCard({ item, isFavorite, isWatched, onFavorite, onOpen }: MediaCardProps) {
  return (
    <article onClick={onOpen} className="group cursor-pointer rounded-[24px] border border-line bg-white p-4 shadow-card transition hover:-translate-y-0.5 hover:border-[#C9C8FA] hover:shadow-lg">
      <div className="relative h-[240px] overflow-hidden rounded-[18px] bg-[#F6F3FF] bg-cover bg-center" style={{ backgroundImage: `url(${item.posterUrl ?? "/media-poster-placeholder.svg"})` }}>
        ...
        <button type="button" onClick={(event) => { event.stopPropagation(); onFavorite(); }} aria-label={isFavorite ? `取消收藏 ${item.title}` : `收藏 ${item.title}`} className={`absolute right-3 top-3 grid size-9 place-items-center rounded-xl bg-white/85 transition hover:bg-white ${isFavorite ? "text-accent" : "text-muted"}`}><Bookmark size={16} fill={isFavorite ? "currentColor" : "none"} /></button>
      ...
      <button type="button" onClick={(event) => { event.stopPropagation(); onOpen(); }} aria-label={`查看${item.title}详情`} className="mt-4 w-full text-left"><div className="flex items-center gap-2 text-xs text-muted"><CalendarDays size={13} />{item.releaseDate ?? "上映时间暂无公开资料"}<span>·</span><span>{item.genres.slice(0, 2).join(" / ") || "类型待补充"}</span></div><p className="mt-3 line-clamp-2 text-sm leading-6 text-muted">{item.overview}</p><div className="mt-4 flex items-center justify-between gap-2"><div className="flex flex-wrap gap-2">{item.genres.slice(0, 3).map((genre) => <span key={genre} className="rounded-full bg-[#F6F3FF] px-2.5 py-1 text-[11px] font-semibold text-[#7169B1]">{genre}</span>)}</div><span className="shrink-0 text-xs font-bold text-accent">查看详情</span></div></button>
    </article>
  );
}
